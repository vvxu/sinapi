...
Voce_Record_Msg = PymongoCRUD(dbname, "voce_record_msg")
...
class MongDBManger:
    def __init__(self, user_id):
        ...
        # 用于记录完整聊天记录
        self.user_voce_record_msg = Voce_Record_Msg
        # 系统S存储的模型
    ...
    # 记录所有记录
    def insert_user_current_voce_record_msg(self, created_at, user, role, msg):
        insert_data = {
            "created_at": created_at,
            "user": user,
            "role": role,
        ...
        self.user_voce_record_msg.insert_one(insert_data)
...
class MessageHandler:
    ...
    def process_user_message(self):
        ...
        if response != "超时错误，请重试！":
            if self.get_user_current_chat_mode() == "1":
                self.mongo_manager.insert_user_current_voce_record_msg(
                    self.created_at, self.user_id, "assistant", response)
                self.mongo_manager.insert_user_current_voce_display_msg(
                    self.created_at, self.user_id, "assistant", response)
            self.mongo_manager.insert_user_current_voce_record_msg(self.created_at, self.user_id, "assistant", response)
